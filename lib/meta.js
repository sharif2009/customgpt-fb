...
export async function getRecentComments() {
  ...
  await ignoreMetaReadError(() => collectPagePostComments(pageId, commentsById));
  await ignoreMetaReadError(() => collectAdStoryComments(commentsById));
...
export async function getRecentPosts() {
  ...
  await ignoreMetaReadError(() => collectPosts(`${pageId}/feed`, postsById, "feed"));
  await ignoreMetaReadError(() => collectPosts(`${pageId}/posts`, postsById, "post"));
  await ignoreMetaReadError(() => collectAdStoryPosts(postsById));
...
async function collectAdStoryPosts(postsById) {
  ...
  for (const storyId of storyIds) {
    const post = await ignoreMetaReadError(() =>
      metaGet(storyId, {
        fields: "id,message,story,created_time,permalink_url,comments.summary(true).limit(0)"
      })
    );
...
async function collectAdStoryComments(commentsById) {
  ...
  for (const storyId of storyIds) {
    const commentData = await ignoreMetaReadError(() =>
      metaGet(`${storyId}/comments`, {
...
async function ignoreMetaReadError(fn) {
  try {
    return await fn();
  } catch (error) {
    return null;
